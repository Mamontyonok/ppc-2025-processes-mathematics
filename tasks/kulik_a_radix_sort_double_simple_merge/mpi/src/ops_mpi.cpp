...
std::vector<double> KulikARadixSortDoubleSimpleMergeMPI::SimpleMerge(const std::vector<std::vector<double>>& sorted_arrays) {
...
void KulikARadixSortDoubleSimpleMergeMPI::LSDSortDouble(std::vector<double>& arr) {
    ...
    MPI_Scatterv(arr.data(), sendcounts.data(), displs.data(), MPI_DOUBLE, local_arr.data(), local_size, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    MPI_Gatherv(local_arr.data(), local_size_int, MPI_DOUBLE, arr.data(), recv_counts.data(), recv_displs.data(), MPI_DOUBLE, 0, MPI_COMM_WORLD);
    if (proc_rank == 0) {
        std::vector<std::vector<double>> sorted_parts;
        for (int i = 0; i < proc_num; ++i) {
            std::vector<double> part(
                arr.begin() + recv_displs[i],
                arr.begin() + recv_displs[i] + recv_counts[i]
            );
