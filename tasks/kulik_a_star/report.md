...
В последовательном алгоритме мы создаём новым коммуникатор `MPI_Comm star` с помощью функции `MPI_Graph_create`. `star` наследует от `WORLD` все процессы и сохраняет их порядок, но меняет топологию - стандартный коммуникатор `MPI_COMM_WORLD` имеет топологию полного графа. `MPI_Graph_create(MPI_COMM_WORLD, proc_num, index.data(), edges.data(), 0, &star)`, где index - вектор, i-я компонента которого показывает: со сколькими процессами напрямую будет связан i-й процесс; edges - это вектор, в котором начиная с 0 и до `proc_num - 1` будет указано, с какими процессами связан конкретный процесс. Важно понимать, что получившаяся топология - чисто виртуальная, то есть нам не нужно будет отправлять сообщения через центр звезды - мы сразу можем указывать процесс-получатель, сообщение само будет пересылаться через корневой процесс.
